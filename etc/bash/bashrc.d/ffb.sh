...
ffb() {
	...
	find "$dir" \
		...
		while read -r size file; do
			...
			printf "%s%-3d%s %s%-4s%s %s%-18s%s %s%-6s%s %s%s%s\n" \
				"$yellow" "$seq" "$reset" \
				"$red" "$perms" "$reset" \
				"$magenta" "$owner" "$reset" \
				"$green" "$size" "$reset" \
	...
	echo
	echo "=== Parâmetros ==="
	echo -e "Diretório : ${green}${dir}${reset}"
	echo -e "Padrão    : ${green}${pattern}${reset}"
	echo -e "Quantidade: ${green}${limit}${reset}"
	echo
	echo -e "Uso: ${blue}ffb \"*.c\"${reset}, ${blue}ffb \"*.c\" 10${reset}"
	echo -e "     ${blue}ffb . ${reset}"
	echo -e "     ${blue}ffb /var/log 20 ${reset}"
	echo -e "     ${blue}ffb "*.log" ${reset}"
	echo -e "     ${blue}ffb "\\*.c" ${reset}"
}
export -f ffb
